...
class EsplorerAPI(BTCAPI):
    ...
    def get_chain_length(self):
        url = self.base_url + 'blocks/tip/height'
        r = self.pool_manager.request('GET', url)
        height = r.data.decode('utf-8')
        if r.status != 200:
            print(
                'ERROR: get_chain_length() could not retrieve tx_id from the remote API: ',
                r.status,
                r.data.decode('utf-8'))
            exit(-1)
        return int(height)
...
if __name__ == '__main__':
    ...
    print("[CHAIN_LENGTH]", API.get_chain_length())
